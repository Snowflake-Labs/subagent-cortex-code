...
1. Clone the Cortex Code integration repository:
   ```bash
...
2. Run the installation script:
   ...
   This will:
   - Copy shared scripts and security modules to `~/.claude/skills/cortex-code/`
   - Parameterize for Claude Code environment
...
4. **(Optional) Configure security settings:**
   ```bash
   # Edit configuration
   vi ~/.claude/skills/cortex-code/config.yaml
   ```
   
   See [SECURITY_GUIDE.md](SECURITY_GUIDE.md) for configuration recommendations.

5. The skill will automatically load when you mention Snowflake-related tasks.
